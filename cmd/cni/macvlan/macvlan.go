...
func loadConf(bytes []byte) (*conf.NetConf, string, error) {
	n := &conf.NetConf{}
	if err := json.Unmarshal(bytes, n); err != nil {
		return nil, "", fmt.Errorf("failed to load netconf: %v", err)
	}
	if n.Master == "" {
		n.Master = utils.GetNodeMaster()
	}
	if n.Master == "" {
...
func createMacvlan(conf *conf.NetConf, ifName string, netns ns.NetNS, vlanid int) (*current.Interface, error) {
	...
	masterLink, err := vlan.GetVLANLink(conf, vlanid)
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	vlanResult, err := cniextend.ExecAdd(netconf.IPAM.Type, args.StdinData)
	...
	ipamResult, err = vlanResult.ConvertToResult()
	if err != nil {
		return err
	}

	//创建macvlan需要移到调用IPAM之后
	macvlanInterface, err := createMacvlan(netconf, args.IfName, netns, vlanResult.Vlan())
	if err != nil {
		return err
	}
...
func main() {
	skel.PluginMain(cmdAdd, cmdGet, cmdDel, version.All, bv.BuildString("hcmacvlan"))
}
