...
// NetConf is the network config for bridge cni
type NetConf struct {
	types.NetConf
...
func setupVeth(netns ns.NetNS, br *netlink.Bridge, ifName string, mtu int, hairpinMode bool, vlanID int) (*current.Interface, *current.Interface, error) {
	...
	if vlanID != 0 && vlanID != 1 {
		...
		// 查询bond0
		bondVeth, err := netlink.LinkByName("bond0")
		if err != nil {
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	var r cniextend.VlanResult
	var ipamResult current.Result
	if isLayer3 {
		// run the IPAM plugin and get back the config to apply
		r, err := cniextend.ExecAdd(n.IPAM.Type, args.StdinData)
		if err != nil {
			return err
		}
		...
		ipamResult, err = r.ConvertToResult()
		if err != nil {
			return err
		}
	}

	hostInterface, containerInterface, err := setupVeth(netns, br, args.IfName, n.MTU, n.HairpinMode, r.Vlan())
	if err != nil {
		return err
	}
...
func main() {
	skel.PluginMain(cmdAdd, cmdCheck, cmdDel, version.All, "bridge")
}
