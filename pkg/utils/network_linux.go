...
func DoNetworking(
	...
	if conf.ClusterIPCIDR == "" {
		conf.ClusterIPCIDR = os.Getenv("CLUSTERIP_CIDR")
	}

	clusterIPCIRDR, _ := cnitypes.ParseCIDR(conf.ClusterIPCIDR)
	nodeip, err := GetNodeIP()
	if err != nil {
		klog.Errorf("get nodeip error %v", err)
		return "", "", err
	}
	hostnet, _ := cnitypes.ParseCIDR(nodeip.String())
	var routes []*net.IPNet
	routes = append(routes, clusterIPCIRDR)
	newHostDst := &net.IPNet{IP: hostnet.IP, Mask: net.CIDRMask(32, 32)}
	routes = append(routes, newHostDst)

	// Select the first 11 characters of the containerID for the host veth.
	hostVethName = "bifr" + args.ContainerID[:Min(11, len(args.ContainerID))]
	contVethName := "veth1"
	...
	klog.Infof("Setting the host side veth name to %s", hostVethName)
	...
	err = ns.WithNetNSPath(args.Netns, func(hostNS ns.NetNS) error {
		...
		if mac, err := net.ParseMAC("EE:EE:EE:EE:EE:EE"); err != nil {
			...
			if err = netlink.LinkSetHardwareAddr(hostVeth, mac); err != nil {
				logger.Warningf("failed to Set MAC of %q: %v. Using kernel generated MAC.", hostVethName, err)
			}
		}
		...
		logger.Infof("Found MAC for container veth")
		...
		if hasIPv4 {
			...
			for _, r := range routes {
				if r.IP.To4() == nil {
					logger.Info("Skipping non-IPv4 route")
					continue
				}
				logger.Warningf("Adding IPv4 route")
...
func SetupRoutes(hostVeth netlink.Link, result *current.Result) error {
	...
	for _, ipAddr := range result.IPs {
		...
		newDst := &net.IPNet{IP: ipAddr.Address.IP, Mask: net.CIDRMask(32, 32)}
		route := netlink.Route{
...
func configureSysctls(hostVethName string, hasIPv4, hasIPv6 bool) error {
	...
	if err = writeProcSys(fmt.Sprintf("/proc/sys/net/ipv6/conf/%s/accept_ra", hostVethName), "0"); err != nil {
		logger.Warningf("failed to set net.ipv6.conf.%s.accept_ra=0: %s", hostVethName, err)
	}
